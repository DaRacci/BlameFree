...
The current user experience for the review harness is entirely terminal-based: CLI flags for running benchmarks, manually reading JSON files for results, and the Ratatui TUI for live monitoring. This creates friction for quick result browsing, live monitoring at a distance, benchmark launching, and historical comparison. A web dashboard solves all of these with a single browser interface.
