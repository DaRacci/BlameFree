...
- [ ] **1.1 Create `dashboard_event.rs`** — `crates/riv-harness/src/dashboard_event.rs`
  ...
  - **Verify:** Module compiles with `cargo check -p riv-harness`

- [ ] **1.2 Create `dashboard.rs`** — `crates/riv-harness/src/dashboard.rs`
...
- [ ] **2.1 Add Ratatui + Crossterm to riv-harness**
  - **File:** `crates/riv-harness/Cargo.toml`
  ...
  - **Verify:** `cargo check -p riv-harness` succeeds

- [ ] **2.2 Add dashboard feature flag (optional)**
  - **File:** `crates/riv-harness/Cargo.toml`
  - **Change:** Add `[features]` section with `dashboard = ["ratatui", "crossterm"]`
  - **Verify:** `cargo check -p riv-harness --no-default-features` succeeds; `cargo check -p riv-harness --features dashboard` succeeds

## Phase 3: Wire Into CLI

- [ ] **3.1 Add `--dashboard` CLI flag**
  - **File:** `crates/riv-harness/src/config.rs` (or whichever file defines `CliArgs`)
...
- [ ] **4.1 Create dashboard channel and spawn dashboard task in main()**
  - **File:** `crates/riv-harness/src/main.rs`
...
- [ ] **5.3 Emit `AgentStarted` before each agent API call**
  - **File:** `crates/riv-harness/src/main.rs` (inside the agent spawn in `evaluate_pr_single_agent`)
...
- [ ] **6.1 Thread `DashboardChannel` through consensus pipeline**
  - **File:** `crates/riv-consensus/src/lib.rs`
...
- [ ] **9.3 Dashboard with consensus mode**
  ...
  - **Verify:** Dashboard works with consensus pipeline; events flow through `riv-consensus`.
...
- [ ] **10.1 Handle terminal resize events**
  - **File:** `crates/riv-harness/src/dashboard.rs`
