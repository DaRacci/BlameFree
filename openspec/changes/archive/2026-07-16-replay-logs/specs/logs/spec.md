...
*(None — all log viewing backend endpoints and types are newly added.)*

## REMOVED Requirements

- **Replay mode endpoints** (`POST /api/runs/:id/replay`, `GET /api/runs/:id/replay/status`) were dropped. The replay mode concept was not implemented; these endpoints were defined in the archived `openspec/changes/replay-logs/specs/archive/replay/spec.md` and are not part of the current log viewing scope.
