...
## Intent
Create the Cargo workspace skeleton with all crate stubs, set up inter-crate dependencies, and implement the main binary crate (riv-harness) that orchestrates the full benchmark pipeline.
...
## Approach
Cargo workspace at root, crates/ subdirectory, each crate independently compilable. riv-aggregator and riv-auditor have both lib.rs (library) and main.rs (CLI entrypoint). riv-harness depends on all 7 library crates.
