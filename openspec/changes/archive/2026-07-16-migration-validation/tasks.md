...
- [x] Implement git helper functions in `crates/riv-tools/src/lib.rs`: clean_repo (git clean -fdx), checkout_pr (git fetch + checkout), extract_diff (git diff)
- [x] Implement validation in `crates/riv-harness/src/validation.rs`: Baseline struct (serde Deserialize), compute_delta(), threshold checks
- [x] Add `--validate` flag to `crates/riv-harness/src/main.rs` (clap derive)
...
- [x] Create run_ci.sh entrypoint script (cargo build --release -p riv-harness + ./target/release/riv-harness --ci ...)
