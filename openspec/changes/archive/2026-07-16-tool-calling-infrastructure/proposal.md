...
multiple languages, git operations, and diff generation — in a way that is
typed, concurrent, safe, and LLM-friendly. Without this infrastructure, each
tool would be ad-hoc, error handling would be inconsistent, and concurrent
execution would require duplicated boilerplate.

## What Changes

Define a typed, concurrent, and safe infrastructure for wrapping external tools
(linters, git, diff) as Rig Tool trait implementations. Includes LinterTool/GitTool
patterns, TOML-based config, typed error types (LinterError, GitError, ConfigError),
shared timeout handling, and concurrent execution via tokio.
