...
- [ ] Implement validation in `crates/crb-reporting/src/lib.rs`: Baseline struct (serde Deserialize), compute_delta(), threshold checks
- [ ] Add `--validate` flag to `crates/crb-harness/src/main.rs` (clap derive)
...
- [ ] Create run_ci.sh entrypoint script (cargo build --release -p crb-harness + ./target/release/crb-harness --ci ...)
