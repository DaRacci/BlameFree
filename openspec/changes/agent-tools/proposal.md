...
Agents currently review with zero tools — they can't grep, read files, check git history, or search docs. This severely limits their ability to:

- **Static Analysis (SA) agents**: Can't run linter commands, grep for patterns, or inspect files they suspect contain bugs.
- **Code Logic (CL) agents**: Can't build diffs, check blame annotations, or trace control flow through multiple files.
- **Architecture (AR) agents**: Can't inspect directory structure, check imports, or verify module boundaries.
- **Security (SEC) agents**: Can't grep for dangerous patterns (`unsafe`, `eval`, `exec`), check for known-vulnerable dependencies, or examine dependency trees.

Without tools, agents rely entirely on the diff text provided in their prompt, missing critical context that exists in the wider repository.
