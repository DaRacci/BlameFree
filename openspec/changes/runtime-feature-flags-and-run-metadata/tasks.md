...
- [ ] **1.1 Define `RunMetadata` struct** — In `crates/riv-harness/src/metadata.rs`
  - All fields from design.md with `#[serde(default)]`
  - `impl Default` with defaults matching current CLI defaults
- [ ] **1.2 Add `build.rs` for harness commit** — `crates/riv-harness/build.rs`
  - Run `git rev-parse HEAD` → inject as `HARNES_BUILD_COMMIT` env var
  - Graceful fallback when outside git repo
- [ ] **1.3 Export `RunMetadata` from `riv-harness` lib** — `pub use metadata::RunMetadata;`
...
- [ ] **4.1 Add `RuntimeConfig` struct** — In `crates/riv-harness/src/config.rs`
...
- [ ] **4.6 Remove unused feature flags from Cargo.toml** — After all `cfg!()` references are removed
  - `crates/riv-harness/Cargo.toml`: remove `exp14_template_vars`, `exp16_adaptive_agents` features
  - `crates/riv-tools/Cargo.toml`: remove `exp14_template_vars`, `exp14_submit_finding` features
  - `crates/riv-consensus/Cargo.toml`: remove `exp14_submit_finding`, `exp16_adaptive_agents` features
  - `crates/riv-agents/Cargo.toml`: remove `exp14_submit_finding` feature
  - `crates/riv-webui-backend/Cargo.toml`: remove `exp14_template_vars`, `exp16_adaptive_agents` feature references to riv-harness

## Phase 5: Update Dashboard Events

- [ ] **5.1 Add `RunStarted` variant** — `crates/riv-dashboard/src/lib.rs`
  - `RunStarted { metadata: RunMetadata }`
- [ ] **5.2 Extend `RunFinished` variant** — `crates/riv-dashboard/src/lib.rs`
  - Add `metadata: RunMetadata` field
- [ ] **5.3 Update webui `DashboardEvent`** — `crates/riv-webui/src/events.rs`
...
- [ ] **5.6 Add metadata to `ActiveRun`** — `crates/riv-webui/src/server.rs`
  - `metadata: RunMetadata` field on `ActiveRun`

## Phase 6: WebUI Display

- [ ] **6.1 Add metadata to run detail API response** — `crates/riv-webui/src/api/runs.rs`
  - `RunDetail` gains `metadata: Option<RunMetadata>`
- [ ] **6.2 Add metadata component to frontend** — `crates/riv-webui/frontend/src/`
