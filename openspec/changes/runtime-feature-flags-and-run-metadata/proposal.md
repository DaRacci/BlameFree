...
Replace compile-time `#[cfg(feature = "...")]` gates with runtime-configurable boolean flags, and add a structured `RunMetadata` section to benchmark and ad-hoc run outputs to capture the configuration context that produced each run.

## Motivation

### Compile-time flags are inflexible

The project currently uses four feature flags (`reduce-diff`, `template_vars`, `submit_finding`, `adaptive_agents`) to select between alternative code paths at compile time. This means:

- **Deployments are monolithic** — you must build a separate binary for each flag combination (e.g., one binary for `reduce-diff` enabled, one for disabled).
...
- **Feature flag conversion**: Replace all `#[cfg(feature = "...")]` gates in the main workspace with runtime `bool` checks against a `RuntimeConfig` struct. Flags to convert: `reduce-diff`, `template_vars`, `submit_finding`, `adaptive_agents`.
...
- Changing the **implementations** behind the feature gates (the actual logic of `strip_diff_metadata`, `template_vars`, `submit_finding`, `adaptive_agents` remains unchanged).
- Changing pipeline logic, concurrency model, cache architecture, or agent orchestration.
- Converting feature flags in child workspaces (e.g., `workspaces/child-1-exp13/`).
