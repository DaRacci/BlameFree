# Change: Rule System (riv-rules Crate)
...
## Scope
New `crates/riv-rules/` library crate with rule discovery, YAML frontmatter parsing, glob matching, language detection, and preamble formatting. Integration hooks in `riv-agents` (optional `rules_preamble` parameter) and `riv-harness` (CLI flag, startup loading).
