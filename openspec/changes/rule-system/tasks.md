...
- [ ] Create `crates/crb-rules/` directory with `Cargo.toml`:
...
- [ ] Add to workspace — no change needed: `members = ["crates/*"]` already covers it
- [ ] Create `src/lib.rs` with module declarations: `mod parser; mod matcher; mod preamble;`
- [ ] Create `src/parser.rs`, `src/matcher.rs`, `src/preamble.rs` as empty stubs
- [ ] Verify `cargo check -p crb-rules` succeeds

## Phase 2 — Core Types and Parsing

- [ ] Define `Rule` struct in `lib.rs`:
  - Fields: `description: Option<String>`, `globs: Vec<String>`, `always_apply: bool`, `body: String`, `source_file: PathBuf`
  - Derives: `Debug, Clone, Serialize, Deserialize`
- [ ] Define `RuleMetadata` struct in `parser.rs`:
  - Fields: `description: Option<String>`, `globs: Option<GlobsField>`, `always_apply: Option<bool>`
  - Derives: `Debug, Clone, Serialize, Deserialize`
- [ ] Define `GlobsField` untagged enum in `parser.rs`:
  - Variants: `Single(String)`, `Multiple(Vec<String>)`
  - Derives: `Debug, Clone, Serialize, Deserialize`
- [ ] Implement `parse_rule_file()` in `parser.rs`:
...
- [ ] Define `RuleSet` struct in `lib.rs`:
  - Fields: `rules: Vec<Rule>`, `always_rules: Vec<Rule>`, `source_dir: PathBuf`
  - Derives: `Debug, Clone`
- [ ] Implement `RuleSet::load_from_dir()`:
...
- [ ] Implement `detect_repo_languages()` in `matcher.rs`:
  - Collect unique languages from a slice of `PathBuf`
  - Return `HashSet<String>`
- [ ] Implement `RuleSet::matching()`:
...
- [ ] Implement `RuleSet::format_preamble()` in `preamble.rs`:
...
- [ ] Modify `build_agent()` in `crates/crb-agents/src/lib.rs`:
...
- [ ] Update all call sites of `build_agent()` in the workspace to pass `None` (backward compatible)
- [ ] Verify `cargo check` passes across workspace

## Phase 7 — Integration: crb-harness

- [ ] Add `crb-rules` dependency to `crates/crb-harness/Cargo.toml`
...
- [ ] Run full `cargo test -p crb-rules` and verify all tests pass
