...
use riv_agents::prompts;
use riv_cache::filesystem::FilesystemBackend;
...
use riv_webui_shared::review::RunConfig;
...
async fn run_adhoc_review_inner(
    ...
    let cache_dir = output_subdir.join(riv_cache::paths::CACHE_DIR_NAME);
    ...
    let context = riv_harness::eval::EvalContext {
        ...
        pull_request: Some(riv_types::vcs::pr::PrMeta {
    ...
    let cfg = EvalConfig {
        ...
        context,
        strategy: riv_harness::eval::EvalStrategy::Panel,
        model: wrapped_model,
    ...
    let findings = riv_harness::pipeline::evaluate(Diff::new(diff.raw.clone()), &cfg).await?;
...
fn load_adhoc_summary(path: &Path) -> (Option<serde_json::Value>, bool) {
    let summary_path = path.join(riv_harness::paths::SUMMARY_FILE);
