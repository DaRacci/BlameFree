//! Filesystem-backed [`LlmCache`] — content-addressed LLM interaction cache.
//!
//! Cache structure:
//! ```text
//! cache/{run_id}/
...
use crate::cache::types::{
    CacheEntry, CacheIndex, GlobalCacheStats, PrCacheStats, PruneResult, Result, ScrubResult,
};
...
fn load_cache_index(pr_dir: &Path) -> Option<(CacheIndex, String)> {
    let index_path = pr_dir.join(paths::INDEX_FILE);
...
impl LlmCache {
    ...
    pub fn new(base: &Path, pr_key: &str) -> Result<Self> {
        ...
        std::fs::create_dir_all(dir.join(paths::AGENTS_DIR))?;
        std::fs::create_dir_all(dir.join(paths::JUDGE_DIR))?;
        std::fs::create_dir_all(dir.join(paths::CONTEXT_DIR))?;
    ...
    fn index_path(&self) -> PathBuf {
        self.dir.join(paths::INDEX_FILE)
    }
    ...
    pub fn prune(
        ...
        let index_path = base_dir.join(paths::INDEX_FILE);
    ...
    pub fn rebuild(base_dir: &Path, dry_run: bool) -> Result<()> {
        ...
        for (_pr_key, pr_dir) in &dirs {
            ...
            for (old_key, entry_meta) in &idx.entries {
                ...
                let new_key = sha256_hex(&meta_str);
...
mod tests {
    ...
    fn test_compute_agent_key_deterministic() {
        let key1 = crate::cache::compute_agent_cache_key("abc", "def", "gpt-4o", "SA", "rules123");
        let key2 = crate::cache::compute_agent_cache_key("abc", "def", "gpt-4o", "SA", "rules123");
        assert_eq!(key1, key2);
    }
    ...
    fn test_compute_agent_key_different_inputs() {
        let key1 = crate::cache::compute_agent_cache_key("abc", "def", "gpt-4o", "SA", "rules123");
        let key2 = crate::cache::compute_agent_cache_key("abc", "xyz", "gpt-4o", "SA", "rules123");
        assert_ne!(key1, key2);
    }
    ...
    fn test_agent_cache_hit_miss() {
        ...
        let key = crate::cache::compute_agent_cache_key("ph", "dh", "gpt-4o", "SA", "rh");
    ...
    fn test_judge_cache_hit_miss() {
        ...
        let key = crate::cache::compute_judge_cache_key("jph", "finding", "golden", "judge-model");
    ...
    fn test_context_cache_hit_miss() {
        ...
        let key = crate::cache::compute_context_cache_key("gph", "dh", "rh", "gpt-4o");
    ...
    fn test_cache_key_hex_length() {
        let key = crate::cache::compute_agent_cache_key("abc", "def", "model", "SA", "rules");
    ...
    fn test_cache_stats_basic() {
        ...
        }
        {
            ...
            let key3 = crate::cache::compute_context_cache_key("gph", "dh", "rh", "m");
            cache2
    ...
    fn test_cache_prune_dry_run() {
        ...
        // Create a PR with some entries
        {
            ...
            let key2 = crate::cache::compute_agent_cache_key("c", "d", "m", "SA", "r");
    ...
    fn test_cache_prune_max_age() {
        ...
        {
            let cache = LlmCache::new(base.path(), "test-pr").unwrap();
            key1 = crate::cache::compute_agent_cache_key("a", "b", "m", "SA", "r");
            cache.save_agent_cached(&key1, "SA", "p", "r").unwrap();
            key2 = crate::cache::compute_agent_cache_key("c", "d", "m", "SA", "r");
            cache.save_agent_cached(&key2, "SA", "p", "r").unwrap();
        }
    ...
    fn setup_test_cache() -> (tempfile::TempDir, String) {
        let base = tempfile::tempdir().unwrap();
        let cache = LlmCache::new(base.path(), "test-pr").unwrap();
        let key = crate::cache::compute_agent_cache_key("a", "b", "m", "SA", "r");
    ...
    fn test_cache_rebuild() {
        ...
        {
            let cache = LlmCache::new(base.path(), "test-pr").unwrap();
            key = crate::cache::compute_agent_cache_key("a", "b", "m", "SA", "r");
            cache
                .save_agent_cached(&key, "SA", "prompt", "response")
                .unwrap();
        }
    ...
    fn test_cache_prune_max_prs() {
        ...
        }
        {
            ...
            let key = crate::cache::compute_agent_cache_key("c", "d", "m", "SA", "r");
            cache.save_agent_cached(&key, "SA", "p", "r").unwrap();
        }
        {
            let cache = LlmCache::new(base.path(), "pr-newest").unwrap();
            let key = crate::cache::compute_agent_cache_key("e", "f", "m", "SA", "r");
            cache.save_agent_cached(&key, "SA", "p", "r").unwrap();
        }
