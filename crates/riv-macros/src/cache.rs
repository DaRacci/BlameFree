...
pub fn derive_cachable_impl(input: &DeriveInput) -> TokenStream {
    ...
    let into_ref_fields: Vec<_> = cache_key_fields
        ...
        .chain(cache_ref_fields.iter().map(|(ident, _)| {
            quote! {
                #ident: {
                    let key = ::riv_cache::traits::CacheKey::cache_key(&self.#ident);
                    let serialized = ::serde_json::to_string(&self.#ident)
                        .expect("Cacheable::into_ref serialization failed");
                    ::riv_cache::traits::CacheBackend::store_raw(backend, &key, &serialized);
                    key
                }
            }
        }))
    ...
    let from_ref_fields: Vec<_> = cache_key_fields
      ...
      .chain(cache_ref_fields.iter().map(|(ident, _)| {
          quote! {
              #ident: {
                  let serialized = ::riv_cache::traits::CacheBackend::load_raw(backend, &form.#ident);
    ...
    let cache_key_contributions: Vec<_> = cache_key_fields
        ...
        .chain(cache_ref_fields.iter().map(|(ident, _)| {
            quote! { ::riv_cache::traits::CacheKey::cache_key(#ident) }
        }))
        .collect();
    ...
    let expanded = quote! {
        ...
        impl ::riv_cache::traits::CacheKey for #ref_name {
        ...
        impl ::riv_cache::traits::Cacheable for #name {
            type RefForm = #ref_name;

            fn into_ref(self, backend: &dyn ::riv_cache::traits::CacheBackend) -> Self::RefForm {
                #ref_name {
                    #(#into_ref_fields,)*
                }
            }

            fn from_ref(form: Self::RefForm, backend: &dyn ::riv_cache::traits::CacheBackend) -> Self {
                Self {
                    #(#from_ref_fields,)*
                }
            }
