...
pub fn derive_entity_model_impl(input: &DeriveInput) -> TokenStream {
    ...
    let entity_loader_impl = if has_auto_increment_pk {
        quote! {}
    } else {
        build_entity_loader(struct_name, &entity_name)
    };

    let load_children_impl = build_load_children(&processed, struct_name, has_auto_increment_pk);
...
fn build_load_children(
    processed: &[ProcessedField],
    struct_name: &Ident,
    has_auto_increment_pk: bool,
) -> TokenStream2 {
    ...
    for pf in processed {
        ...
        match relation {
            RelationKind::HasMany {
                ...
                child_blocks.push(if has_auto_increment_pk {
                    quote! {
                        if let Some(pk) = self.id {
                            let children = #child_entity::find()
                                .filter(#column_ty::#fk_column_variant.eq(pk))
                                .all(db).await?;
    ...
    let has_blocks = !child_blocks.is_empty();
    let (db_param, depth_param) = if has_blocks {
        (quote! { db }, quote! { depth })
    } else {
        (quote! { _db }, quote! { _depth })
    };
    let body = if has_blocks {
        quote! {
    ...
    quote! {
        ...
        impl crate::stor::LoadChildren for #struct_name {
            async fn load_children(
                &mut self,
                #db_param: &::sea_orm::DatabaseConnection,
                #depth_param: crate::stor::LoadDepth,
            ) -> Result<(), ::anyhow::Error> {
                #body
            }
        }
