...
pub fn derive_entity_model_impl(input: &DeriveInput) -> TokenStream {
    ...
    let has_auto_increment_pk = processed
        .iter()
    ...
    let missing_from_model = has_pure_relations && !has_flatten; // From<Model> not generated
    let entity_loader_impl = if has_auto_increment_pk || missing_from_model {
        quote! {}
    } else {
        build_entity_loader(struct_name, &entity_name)
    };

    // Only generate LoadChildren when EntityLoader is also generated,
    // because LoadChildren: EntityLoader is a trait bound.
    let entity_loader_needed = !has_auto_increment_pk && !missing_from_model;
    let has_relations_for_children = entity_loader_needed
        && processed.iter().any(|pf| {
            matches!(
                pf.relation,
            ...
            ) || matches!(pf.relation, Some(RelationKind::HasOne { .. }))
        });
    let load_children_impl = if has_relations_for_children {
        build_load_children(&processed, struct_name)
    } else {
        quote! {}
    };
    ...
    quote! {
        ...
        impl crate::stor::EntityLoader for #struct_name {
            async fn load_by_id(
                ...
                id: &mti::prelude::MagicTypeId,
            ) -> Result<Option<Self>, ::anyhow::Error> {
                use ::sea_orm::EntityTrait;
                let row = #entity_name::find_by_id(id.to_string())
                    .one(db)
                    .await?;
            ...
            async fn load_all(
                ...
                use ::sea_orm::EntityTrait;
                let rows = #entity_name::find().all(db).await?;
...
fn build_load_children(processed: &[ProcessedField], struct_name: &Ident) -> TokenStream2 {
    let mut child_blocks: Vec<TokenStream2> = Vec::new();

    for pf in processed {
        let field_ident = &pf.ident;
        let Some(ref relation) = pf.relation else {
            continue;
        };
        ...
        match relation {
            RelationKind::HasMany {
                ...
                let domain_name = entity_alias.strip_suffix("Entity").unwrap_or(entity_alias);
                let child_domain = Ident::new(domain_name, Span::call_site());
                let column_ty = format_ident!("{}Column", domain_name);

                // Derive FK column variant
                let default_fk = format!("{}_id", to_snake_case(&struct_name.to_string()));
                let fk_str = child_fk.as_deref().unwrap_or(&default_fk);
                let fk_column_variant = Ident::new(&snake_to_pascal(fk_str), Span::call_site());

                child_blocks.push(quote! {
                    {
                        let children = #child_entity::find()
                            .filter(#column_ty::#fk_column_variant.eq(self.id.to_string()))
                            .all(db).await?;
                        let mut result = Vec::with_capacity(children.len());
                        for cm in children {
                            let mut child: #child_domain = cm.into();
                            if matches!(depth, crate::stor::LoadDepth::Deep) {
                                child.load_children(db, depth).await?;
                            }
                            result.push(child);
                        }
                        self.#field_ident = result;
                    }
                });
            }
            RelationKind::HasMany {
            ...
            RelationKind::HasOne { entity_alias } => {
                let child_entity = Ident::new(entity_alias, Span::call_site());
                let domain_name = entity_alias.strip_suffix("Entity").unwrap_or(entity_alias);
                let _child_domain = Ident::new(domain_name, Span::call_site());
                let column_ty = format_ident!("{}Column", domain_name);

                // Default FK column: {parent_snake}_id
                let default_fk = format!("{}_id", to_snake_case(&struct_name.to_string()));
                let fk_column_variant =
                    Ident::new(&snake_to_pascal(&default_fk), Span::call_site());

                child_blocks.push(quote! {
                    {
                        let child = #child_entity::find()
                            .filter(#column_ty::#fk_column_variant.eq(self.id.clone()))
                            .one(db).await?;
                        self.#field_ident = child.map(Into::into);
                    }
                });
            }
            RelationKind::BelongsTo { .. } => {}
