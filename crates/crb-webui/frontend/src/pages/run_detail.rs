                    ...
                    view! {
                        ...
                        <div class="table-wrapper">
                            <table class="table">
                                ...
                                <tbody>
                                    {move || results_clone2.iter().map(|pr: &PrResult| {
                                        let pr_badge = match pr.status.as_deref() {
                                        ...
                                        view! {
                                            <tr class="table__row">
                                                <td class="table__td" style="font-weight: var(--weight-semibold, 600);">{format!("#{}", pr.pr_number)}</td>
                                                <td class="table__td">{&pr.title}</td>
                                                <td class="table__td" style="font-family: var(--font-mono, monospace);">{pr.f1.map(|v| format!("{:.3}", v)).unwrap_or_else(|| "—".into())}</td>
                                                <td class="table__td" style="font-family: var(--font-mono, monospace);">{pr.precision.map(|v| format!("{:.3}", v)).unwrap_or_else(|| "—".into())}</td>
                                                <td class="table__td" style="font-family: var(--font-mono, monospace);">{pr.recall.map(|v| format!("{:.3}", v)).unwrap_or_else(|| "—".into())}</td>
                                                <td class="table__td" style="font-family: var(--font-mono, monospace);">{pr.cost.map(|v| format!("${:.4}", v)).unwrap_or_else(|| "—".into())}</td>
                                                <td class="table__td">
