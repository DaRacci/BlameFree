//! Enables hierarchical structs to be flattened to/from flat column-name->value maps for DB storage.
...
pub trait FlattenedStruct: Sized {
    ...
    ///
    /// The column_name strings are `'static` via `Box::leak`.
    /// Safe because they are a finite, fixed set derived from struct field names and attributes.
    ...
    fn unflatten(prefix: &str, cols: &HashMap<String, Option<String>>) -> Self;
}
