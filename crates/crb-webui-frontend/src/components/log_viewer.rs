...
pub fn LogViewer(logs: LogsListResponse, run_id: String) -> impl IntoView {
    ...
    let has_cache = logs.cache_available;
    let has_prs = !logs.prs.is_empty();
                                ...
                                <details style="margin-bottom: 0.5rem;">
                                    ...
                                    <div style=style_pr_body>
                                        {agents.iter().map(|agent| {
                                            ...
                                            let (agent_log, set_agent_log) = signal::<Option<AgentLogResponse>>(None);
                                            let (fetching, set_fetching) = signal(false);
                                            let (fetched, set_fetched) = signal(false);
