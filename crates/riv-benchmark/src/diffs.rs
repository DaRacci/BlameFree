...
pub fn run(benchmark_dir: &Path) -> Result<()> {
    ...
    if !base_repos_dir.exists() {
        bail!(
            "Base repos directory does not exist: {}. Run `riv-benchmark scaffold` first.",
    ...
    if diff_count == 0 && skipped_count == 0 {
        info!("No diffs extracted. Run `riv-benchmark scaffold` first.");
...
mod tests {
    use super::*;
    use riv_shared::test_utils::{setup_empty_commit_repo, setup_repo_with_diffs};
