...
use riv_benchmark::judge::build_judge;
use riv_benchmark::{
...
use riv_types::benchmark::standalone::Benchmark;
...
#[derive(Debug, Parser)]
#[command(name = "riv-benchmark", about = "Benchmark preparation CLI")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
async fn run_benchmark(
    ...
    let client = riv_shared::build_client()?;
    let judge = build_judge(&client, &judge_model);

    let linter_configs = if linter_path.exists() && !skip_linters {
        riv_tools::linters::config::load_linter_config(&linters_config)?
    } else {
        None
    };

    let ruleset = if !skip_rules {
        let rules_dir = Path::new(".riv/rules/");
    ...
    for pr in prs_to_evaluate {
        let diff_str = match parse_github_url(&pr.url) {
            Ok((owner, repo, pr_num)) => {
                let d = riv_benchmark::diff_cache::load_cached_diff(
                    &benchmark_dir,
                    &owner,
                    &repo,
                    pr_num,
